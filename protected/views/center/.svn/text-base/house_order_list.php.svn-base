...
<div id="order-results-list">
               <div class="seller-order-list  product-order-list">
                      <div  class="filter">
                       <form action="<?php echo $this->createUrl('center/house');?>" method="get">
              ...
              <?php  foreach( $h_order->getData() as $item):?>
						...
						<tr  class="order_<?php echo $item->order_code;?>">
							 <td colspan="7" class="p0"><p class="item-title mt10 brb0">订单编号：<?php echo $item->order_code;?><span class="indent20">下单时间：<?php echo date('Y-m-d H:i:s',$item->created);?></span></p></td>
						 </tr>
						 ...
						 <?php $orderinfo=json_decode($val->json);?>
							<tr class="order_<?php echo $item->order_code;?>">
							   <td class="name txtl">
								   ...
								   <a href="<?php echo $this->createUrl('goods/index',array('id'=>$val->goods_id));?>" target="_blank" title="<?php echo $orderinfo->title;?>"><?php echo mb_substr($orderinfo->title, 0, 6);?></a>
							   </td>
							   <td class="seller">
						...
						<?php  endforeach;?>
							...
							<?php  endforeach;?>
                                   ...
                                   <?php
$this->widget('application.widgets.PageToolbar' , array('pagination'=>$h_order->pagination, 'ajaxContainerId'=>'order-results-list','useAjax'=>true, 'route'=>'center/companionhouseorder'));
?>
               </div>
               </div>