                <div class="review-content">
                    <ul class="talk-list">
                    <?php foreach ( $model->ReviewInfo['reviews'] as $row ) : ?>
                        <li>
                            ...
                            <div class="talk-content">
                                <p class="talk-user-info"><?php echo($row['title']);?>
                				  <span class="favorite-grade1"><?php echo($row['satisfaction_face_0']);?>
					                ...
					                <span class="tooltip-con cmt-tooltip">
		                              ...
		                              <label>准时出行：</label><?php echo($row['satisfaction_img_3']);?>
						              <span><?php echo($row['satisfaction_msg_3']);?></span><br />
		                              <label>性价比: </label>：</label><?php echo($row['satisfaction_img_4']);?>
                                ...
                                <div class="talk-info" >
                                    ...
                                    <p class="talk-function" product_review_id="<?php echo $row['product_review_id']; ?>">
                                    <span class="tip" id="helpful_tip_<?php echo($row['product_review_id']);?>"><?php echo Yii::t('info', '此评论对我'); ?></span>
                                    <a href="javascript:;" class="helpful_yes_counter helpful_<?php echo($row['product_review_id']);?>" ><?php echo Yii::t('info', '有用'); ?>(<label id="helpful_yes_<?php echo($row['product_review_id']);?>"><?php echo($row['helpful_yes_counter']);?></label>)</a>
                                    <a href="javascript:;" class="helpful_no_counter helpful_<?php echo($row['product_review_id']);?>" ><?php echo Yii::t('info', '没用'); ?>(<label id="helpful_no_<?php echo($row['product_review_id']);?>"><?php echo($row['helpful_no_counter']);?></label>)</a>
                                    <a href="javascript:;" class="reply login-to-reply-form" ><?php echo Yii::t('info', '回复'); ?>(<?php echo(count($row['replies']));?>)</a>
                                    </p>
                                </div>
                                <div class="comment-form undis" id="comment_<?php echo($row['product_review_id']);?>">
                            <form>
                                <textarea name="" class="comment-txt"></textarea>
                                <a class="btn send-comment" data-id="<?php echo $row['product_review_id']; ?>" href="javascript:;" >评论</a>
                                <!--addReviewReply(<?php echo($row['product_review_id']); ?>)-->
                            </form>
                        </div>
    		<?php if ($row['replies']) : ?>
    		<!--回复列表-->
...
<?php
$this->widget('application.widgets.PageToolbar' , array('pagination'=>$model->ReviewInfo['pagination'], 'ajaxContainerId'=>'results-list', 'useAjax'=>true, 'route'=>'product/reviewPages'));
?>
</div>
