<script type="text/javascript" src="<?php echo Yii::app()->request->baseUrl; ?>/js/product_review.js"></script>
<script type="text/javascript">
$(function(){
    $('.review_submit').live("click",function(){
        var cmtform = $("#cmt-form");
        var obj = $(this);
        $.post('<?php echo Yii::app()->createUrl('product/AjaxPostQualification'); ?>',{product_id:obj.attr("product_id")},function(data){
            if(data.state == 1){
                cmtform.slideToggle();
            }else{
...
function visitor(id){
               $.ajax({
                'url':'<?php echo Yii::app()->createUrl('product/arrivedProduct'); ?>',
                'type':'post',
                'data':'id='+id,
...
function addReviewHelpful()
{
    ...
    $.ajax({
        'url':'<?php echo Yii::app()->createUrl('product/AjaxPostReviewHelpful'); ?>',
        'type':'post',
        'dataType':'json',
        'data':'product_review_id='+helpful_review_id+'&helpful='+helpful_yes_no+'&YII_CSRF_TOKEN='+csrfToken,
...
$(".send-comment").live("click",function(){
    ...
    var csrfToken = $('#csrfToken').val();
    var product_id = $('#submitReviewBtn').attr('product_id');
    var review_id = $(this).data("id");
    if ( review_content == '' ) return msgBoxShow("0",'请输入回复内容！');
    $.post("<?php echo Yii::app()->createUrl('product/AjaxPostReviewReply'); ?>",{parent_review_id:review_id,product_id:product_id,review_title:review_id,review_content:review_content,YII_CSRF_TOKEN:csrfToken},function(json){
        if (json.code == 1){
...
//    var csrfToken = $('#csrfToken').val();
//    var product_id = $('#submitReviewBtn').attr('product_id');
//	var review_content = $('#comment-txt').val();
//	if ( review_content == '' ) return alert('请输入回复内容！');
//
//	$.ajax({
//		type:"POST",
//		dataType:"json",
//		data:{"parent_review_id":review_id,"product_id":product_id,"review_title":review_id,"review_content":review_content,"YII_CSRF_TOKEN":csrfToken},
//		url:"<?php //echo Yii::app()->createUrl('product/AjaxPostReviewReply'); ?>",
//		success:function(json){
//		    $("#comment_"+review_id).hide();
//            if (json.code == 1)
//            {
                    ...
                    <div class="zyxbox-content">
                        <div class="review-head-wrap">
                            <div class="review-head-left">
                                <p class="favorite">
                                    <span class="favorite-grade"><?php echo($model->ReviewInfo['satisfaction_0']);?>%</span>
                                </p>
                                <p class="landscape">
                                    <span>满意度: </span><?php echo($model->ReviewInfo['satisfaction_img_0']);?>
                                </p>
                            </div>
                            <div class="review-head-right">
                                <ul>
                                    <li>
                                        <p class="landscape">
                                            <span>符合描述: </span><?php echo($model->ReviewInfo['satisfaction_img_1']);?>
                                            <span class="indent20">交流沟通: </span><?php echo($model->ReviewInfo['satisfaction_img_2']);?>
                                        </p>
                                    </li>
                                    <li>
                                        <p class="landscape">
                                            <span>准时出行: </span><?php echo($model->ReviewInfo['satisfaction_img_3']);?>
                                            <span class="indent20">性价比:</span><?php echo($model->ReviewInfo['satisfaction_img_4']);?>
                                        </p>
                                        <a href="javascript::void(0)" class="zyxbtn1 review_submit" product_id="<?php echo $model->product_id; ?>">发表评论</a>
                                    </li>
                                </ul>
                            </div>
                        </div>
<!--评论框-->
	<div class="cmt-form" id="cmt-form">
		...
		$form = $this->beginWidget('CActiveForm', array(
			'id' => 'WriteReviewForm',
			'action' => Yii::app()->createUrl('product/AjaxPostReview'),
		...
		<p class="txtc">
			...
			function submitReview()
			{
                ...
                $_p=$('#submitReviewBtn').attr('product_id');
				$s1 = $('#starsResult1').val();
				$s2 = $('#starsResult2').val();
                $s3 = $('#starsResult3').val();
                $s4 = $('#starsResult4').val();
                ...
                $.ajax({
                    'url':'<?php echo Yii::app()->createUrl('product/AjaxPostReview'); ?>',
                    'type':'post',
                    'dataType':'json',
                    'data':'product_id='+$_p+'&review_title='+$_t+'&review_content='+$_r+'&starsResult1='+$s1+'&starsResult2='+$s2+'&starsResult3='+$s3+'&starsResult4='+$s4+'&YII_CSRF_TOKEN='+csrfToken,
                    'success':function(json){
                        //if (json.code == 1)
//			            {
//                        if($('#msg-box').length){
...
//                        $('#cmt-form').toggle();
			...
			<a class="tffbtn2" href="javascript:submitReview();" id="submitReviewBtn" product_id="<?php echo $model->product_id; ?>">发表评论!</a>
		</p>
		<?php $this->endWidget();?>
