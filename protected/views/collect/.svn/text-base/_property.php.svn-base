		...
		<div class="collection-list">
			...
			<?php foreach($image as $val):?>
			 <?php $count++;?>
			  <?php if( $count==1 ):?>
				<li class="big-img">
					<a href="<?php echo  $this->createUrl('goods/index',array('id'=>Property::model()->getPropertyGoods_id($item['object_id'])));?>">
			...
			<p class="operating">
			 ...
			 <?php if(!SiteShare::checkIsShare($item['object_type'],$item['object_id'])):?>
				...
				<?php endif;?>
					...
					<a href="<?php echo $this->createUrl('property/info',array('property_id'=>$item['object_id']));?>" class="edit">引用</a>
				</span>
			</p>
